...
(defn dijkstra-search
  ([rows, columns, matrix]
    (let [current-value (apply min (get-column rows columns 0 matrix))                                                 ;Assigns current value and current position using
      ...
      (dijkstra-search rows columns matrix current-position current-value 0 current-value (cons  current-value '() ))))

  ([rows, columns, matrix, current-position, current-value, position, total, path]                                        ;Iterates through each column, and decreases the
    (if (= (+ 1 position) columns)                                                                                     ;row count each time.
      (assoc {} :total total :path (reverse path))
      ...
      (let [next-value (apply min (get-possible-paths current-position, position, matrix, rows, columns))
        ...
        (recur rows columns matrix next-position next-value (+ position 1) (+ total next-value) (cons next-value path)))
      )
    )
  )
